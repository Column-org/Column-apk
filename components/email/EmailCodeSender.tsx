...
interface EmailCodeSenderProps {
    code: string
    tokenName: string
    tokenSymbol: string
    amount: string
...
export default function EmailCodeSender({ code, tokenName, tokenSymbol, amount }: EmailCodeSenderProps) {
    const { user } = usePrivy()
    const [email, setEmail] = useState('')
    const [loading, setLoading] = useState(false)
    const [status, setStatus] = useState<{ type: 'success' | 'error'; message: string } | null>(null)

    // Backend URL - using local IP for testing on physical device
    const backendUrl = 'http://10.148.185.245:3000/api'
    ...
    const handleSendEmail = async () => {
        ...
        try {
            ...
            console.log('[EmailCodeSender] Response:', { status: response.status, data })

            if (!response.ok) {
                // Show user-friendly message for rate limiting
                if (response.status === 429) {
                    throw new Error('Daily limit reached. You can only send 12 codes per day.')
                }
                console.error('[EmailCodeSender] Error response:', data)
            ...
            console.error('[EmailCodeSender] Error sending email:', error)
            console.error('[EmailCodeSender] Error details:', {
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.description}>
                Send {amount} {tokenSymbol} transfer code to your friend's email
            </Text>
